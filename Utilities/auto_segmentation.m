...
for iSlice=1:length(interleavedSlices_index)
    ...
    if any(interleavedSlices_index(iSlice)==motion_corrupted_slices)
        ...
        Fetal_Brain_zp(:,:,index:index+ceil(SliceThickness/SubunitRes)-1) = Fetal_Brain_moved_zp(:,:,index:index+ceil(SliceThickness/SubunitRes)-1);
        ...
        if iSlice < length(interleavedSlices_index)
            for nextSlice=iSlice+1:length(interleavedSlices_index)
                ...
                Fetal_Brain_zp(:,:,nextSlice_index:nextSlice_index+ceil(SliceThickness/SubunitRes)-1) = Fetal_Brain_moved_zp(:,:,nextSlice_index:nextSlice_index+ceil(SliceThickness/SubunitRes)-1);
            end
        end    
...
% 	disp(['Slice ', num2str(interleavedSlices_index(iSlice)), ' of ', num2str(length(interleavedSlices_index))])
    ...
    for row=1:size(Fetal_Brain_zp,1)
        for col=1:size(Fetal_Brain_zp,2)
            slice_labels = Fetal_Brain_zp(row,col,index:index+ceil(SliceThickness/SubunitRes)-1);
            if unique(slice_labels)==0
                ...
                %frequency in the slice thickness
                    ...
                    elseif any(value(max_index)==Fetal_Brain_zp(row,col,index+ceil(SliceThickness/SubunitRes)))
                        freq_labels_other = freq_labels_other + 1;
                        max_value = Fetal_Brain_zp(row,col,index+ceil(SliceThickness/SubunitRes));
    ...
    %                     max_value = Fetal_Brain_zp(row,col+1,round(index+SliceThickness/SubunitRes-1)/2);
            ...
            Fetal_Brain_zp(row,col,index:index+ceil(SliceThickness/SubunitRes)-1) = max_value;
        end
    end
...
% of acquisition
Simu_Labels = Resize_Volume(Sl_Volume, [round(FOVRead)/SimRes, round(FOVPhase)/SimRes, NbSlices]);
